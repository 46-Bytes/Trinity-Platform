...
interface FileMetadata {
  ...
  openai_file_id?: string;  // OpenAI file ID (legacy)
  llm_file_id?: string;  // LLM provider file ID (Claude, OpenAI, etc.)
  ...
  const uploadFiles = async (diagnosticId: string, filesToUpload: File[]): Promise<FileMetadata[]> => {
    ...
    for (const file of filesToUpload) {
      ...
      results.push({
        ...
        openai_file_id: data.openai_file_id,  // Store OpenAI file ID (legacy)
        llm_file_id: data.llm_file_id,  // Store LLM provider file ID
