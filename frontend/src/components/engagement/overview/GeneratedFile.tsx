...
export interface GeneratedFileProps {
  ...
  generatedAt: Date;
  generatedBy?: string;
  generatedByRole?: string; // Role of user who generated the report (only for admin/firm_admin/super_admin)
  size?: string;
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
            ...
            {generatedByRole && isAdminRole(generatedByRole) && (
              <span className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                <User className="w-3.5 h-3.5" />
                Generated by {formatRoleForDisplay(generatedByRole)}
              </span>
            )}
            {size && (
              <span>{size}</span>
            )}
            {((uploadedByRole && isAdminRole(uploadedByRole)) || (uploadedByAdmin && !uploadedByRole)) && (
              <span className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                <User className="w-3.5 h-3.5" />
                {uploadedByRole ? `Uploaded by ${formatRoleForDisplay(uploadedByRole)}` : 'Uploaded by Admin'}
              </span>
            )}
          </div>
        </div>
