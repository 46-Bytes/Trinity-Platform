...
async def list_generated_documents(
    ...
    """
    List every downloadable deliverable generated by this engagement's follow-up
    tools (BBA, Strategy Workbook, Strategic Business Plan).

    Visible to anyone with access to the engagement (advisor or client), regardless
    of who generated the document. The Trinity Diagnostic report is intentionally NOT
    included here — the frontend lists it separately from the diagnostics it already
    fetches. Only deliverables that are actually exportable are returned.
    """
    ...
    # --- BBA deliverables (report, task planner, presentation) ---
    # Only the most-recently-updated BBA is surfaced, so a reset/new run
    # replaces the prior run's documents rather than piling up.
    bba_service = get_bba_service(db)
    bbas = bba_service.get_bbas_by_engagement(engagement_id)
    bba = max(bbas, key=lambda r: r.updated_at) if bbas else None
    if bba:
        generated_at = _iso(bba.updated_at, bba.created_at)
        client_label = bba.client_name or engagement.engagement_name or "Client"
        if bba.expanded_findings:
            documents.append(GeneratedDocumentItem(
                id=f"bba-report-{bba.id}",
                tool="BBA",
                name=f"{client_label} - Findings & Recommendations.docx",
                file_type="docx",
                generated_at=generated_at,
                download_url=f"/api/poc/{bba.id}/export/docx",
                download_method="POST",
            ))
        if bba.task_planner_tasks:
            documents.append(GeneratedDocumentItem(
                id=f"bba-tasks-{bba.id}",
                tool="BBA",
                name=f"{client_label} - Task Planner.xlsx",
                file_type="xlsx",
                generated_at=generated_at,
                download_url=f"/api/poc/{bba.id}/tasks/export/excel",
                download_method="POST",
            ))
        if bba.presentation_slides and bba.presentation_slides.get("slides"):
            documents.append(GeneratedDocumentItem(
                id=f"bba-pptx-{bba.id}",
                tool="BBA",
                name=f"{client_label} - Presentation.pptx",
                file_type="pptx",
                generated_at=generated_at,
                download_url=f"/api/poc/{bba.id}/presentation/export/pptx",
                download_method="POST",
            ))

    # --- Strategy Workbook deliverable (latest run only) ---
    workbook_service = get_strategy_workbook_service(db)
    workbooks = workbook_service.get_workbooks_by_engagement(engagement_id)
    workbook = max(workbooks, key=lambda r: r.updated_at) if workbooks else None
    if workbook:
        # The workbook file exists once generated_workbook_path is set (status
        # becomes "completed" at that point — "ready" is only the pre-generation
        # state). Reset nulls this path, so the entry vanishes on Start Fresh.
        if workbook.generated_workbook_path:
            documents.append(GeneratedDocumentItem(
                id=f"workbook-{workbook.id}",
                tool="Strategy Workbook",
                name="Strategy Workshop Workbook.xlsx",
                file_type="xlsx",
                generated_at=_iso(workbook.completed_at, workbook.updated_at, workbook.created_at),
                download_url=f"/api/strategy-workbook/{workbook.id}/download",
                download_method="GET",
            ))

    # --- Strategic Business Plan deliverables (plan, employee variant, presentation) — latest run only ---
    sbp_service = get_sbp_service(db)
    plans = sbp_service.get_plans_by_engagement(engagement_id)
    plan = max(plans, key=lambda r: r.updated_at) if plans else None
    if plan:
        generated_at = _iso(plan.completed_at, plan.updated_at, plan.created_at)
        client_label = plan.client_name or engagement.engagement_name or "Client"
        if plan.final_plan:
            documents.append(GeneratedDocumentItem(
                id=f"sbp-plan-{plan.id}",
                tool="Business Planner",
                name=f"{client_label} - Strategic Business Plan.docx",
                file_type="docx",
                generated_at=generated_at,
                download_url=f"/api/strategic-business-plan/{plan.id}/export/docx",
                download_method="GET",
            ))
        if plan.employee_plan:
            documents.append(GeneratedDocumentItem(
                id=f"sbp-employee-{plan.id}",
                tool="Business Planner",
                name=f"{client_label} - Employee Strategy Document.docx",
                file_type="docx",
                generated_at=generated_at,
                download_url=f"/api/strategic-business-plan/{plan.id}/export/employee-docx",
                download_method="GET",
            ))
        if plan.presentation_slides:
            documents.append(GeneratedDocumentItem(
                id=f"sbp-pptx-{plan.id}",
                tool="Business Planner",
                name=f"{client_label} - Presentation.pptx",
                file_type="pptx",
                generated_at=generated_at,
                download_url=f"/api/strategic-business-plan/{plan.id}/presentation/export",
                download_method="GET",
            ))
